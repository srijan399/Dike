                ...
                <div>
                    <CardHeader className="pb-4">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-xl font-bold text-white">Prediction Details</CardTitle>
                            <Button variant="ghost" size="sm" onClick={onClose} aria-label="Close details modal">
                                <X className="w-4 h-4" />
                            </Button>
                        </div>
                        <p className="text-sm text-cyan-100">All fields from the Prediction interface.</p>
                    </CardHeader>
...
function Detail({ label, value, mono, long }: { label: string; value: string; mono?: boolean; long?: boolean }) {
    return (
        <div className="p-3 rounded border border-white/20 bg-white/5">
            <div className="text-xs text-yellow-200 mb-1">{label}</div>
            <div className={`text-sm text-white ${mono ? "font-mono break-all" : ""} ${long ? "text-pretty" : "truncate"}`}>{value || "—"}</div>
        </div>
    )
}
