...
// Contract address
// Contract addresses
const CONTRACT_ADDRESS = '0x5Fe9A48aaD87824d0DCa6A4A0107d435853fd9a3';
const PYUSD_ADDRESS = '0xCaC524BcA292aaade2DF8A05cC58F0a65B1B3bB9'; // PyUSD on Sepolia

// PyUSD ABI (minimal for approval and balance checking)
const PYUSD_ABI = [
...
export default function ContractTestPage() {
  ...
  const [pyUSDContract, setPyUSDContract] = useState<ethers.Contract | null>(null);
  ...
  const connectWallet = async () => {
    try {
      if (typeof (window as any).ethereum !== 'undefined') {
        ...
        console.log('PyUSD contract initialized:', PYUSD_ADDRESS);
      } else {
        alert('Please install MetaMask!');
      }
    } catch (error) {
      console.error('Error connecting wallet:', error);
      alert('Failed to connect to Sepolia testnet. Please check your wallet settings.');
    }
  };
  ...
  const checkAndApproveTokens = async (amount: string) => {
    ...
    try {
      ...
      if (allowance < amountWei) {
        ...
        const approveTx = await pyUSDContract.approve(CONTRACT_ADDRESS, amountWei);
  ...
  const getUpdatedPredictionData = async (predictionId: number) => {
    ...
    try {
      ...
      const userTotalInvestment = await contract.getUserTotalInvestmentInPrediction(account, predictionId);
      ...
      const predictionWithPrices = await contract.getPredictionWithPrices(predictionId);
  ...
  const updateTestResult = (name: string, status: 'pending' | 'success' | 'error', result?: any, error?: string, gasUsed?: string) => {
    setTestResults(prev => {
      const existing = prev.find(t => t.name === name);
      if (existing) {
        return prev.map(t => t.name === name ? { ...t, status, result, error, gasUsed } : t);
      } else {
  ...
  const testBasicContractInfo = async () => {
    ...
    try {
      ...
      updateTestResult('MINIMUM_LIQUIDITY', 'success', formatUSDC(minLiquidity));
    } catch (error: any) {
  ...
  const testViewFunctions = async () => {
    ...
    try {
      ...
      updateTestResult('getActivePredictions', 'success', activePredictions.map((id: bigint) => id.toString()));
    } catch (error: any) {
    ...
    try {
      ...
      if (predictionCount > 0) {
        ...
        try {
          ...
          updateTestResult('getPredictionWithPrices', 'error', null, error.message);
        }
        ...
        try {
          ...
          const totalLiquidity = await contract.getTotalLiquidity(predictionId);
          updateTestResult('getTotalLiquidity', 'success', formatUSDC(totalLiquidity));
        } catch (error: any) {
        ...
        try {
          ...
          const investments = await contract.getPredictionInvestments(predictionId);
          updateTestResult('getPredictionInvestments', 'success', investments.length.toString() + ' investments');
        } catch (error: any) {
          updateTestResult('getPredictionInvestments', 'error', null, error.message);
        }

        // Test getUserInvestmentsInPrediction
        try {
          updateTestResult('getUserInvestmentsInPrediction', 'pending');
          const userInvestments = await contract.getUserInvestmentsInPrediction(account, predictionId);
          updateTestResult('getUserInvestmentsInPrediction', 'success', userInvestments.length.toString() + ' user investments');
        } catch (error: any) {
          updateTestResult('getUserInvestmentsInPrediction', 'error', null, error.message);
        }

        // Test getUserTotalInvestmentInPrediction
        try {
          updateTestResult('getUserTotalInvestmentInPrediction', 'pending');
          const totalInvestment = await contract.getUserTotalInvestmentInPrediction(account, predictionId);
          ...
          updateTestResult('getUserTotalInvestmentInPrediction', 'error', null, error.message);
        }
      } else {
        updateTestResult('Prediction Functions', 'error', null, 'No predictions exist to test');
      }
    } catch (error: any) {
  ...
  const testStateChangingFunctions = async () => {
    ...
    try {
      ...
      if (predictionCount > 0) {
        ...
        const receipt = await tx.wait();
        updateTestResult('investInPrediction', 'success', {
          txHash: tx.hash,
          amount: formatUSDC(amount),
          side: side ? 'Yes' : 'No'
        }, undefined, receipt.gasUsed.toString());
      } else {
        updateTestResult('investInPrediction', 'error', null, 'No predictions exist to invest in');
      }
    } catch (error: any) {
  ...
  const testInvestmentFunctions = async () => {
    ...
    try {
      ...
      if (predictionCount > 0) {
        ...
        console.log(`Investing ${formatUSDC(amount)} in prediction ID ${predictionId} (${side ? 'Yes' : 'No'} side)...`);
        const tx = await contract.investInPrediction(
          predictionId,
          amount,
          side,
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-center mb-8">MultiversePrediction Contract Test Suite</h1>
        <p className="text-center text-gray-600 mb-8">Testing on Sepolia Testnet</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {isConnected ? (
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600">Contract: {CONTRACT_ADDRESS}</p>
            </div>
          ) : (
            <div>
                ...
                >
                  ...
                  {test.result && (
                    <div className="mb-2">
                      ...
                      {test.name === 'investInPrediction' && test.result.updatedPredictionData ? (
                        <div className="space-y-3">
                          {/* Investment Details */}
                          <div className="bg-blue-50 p-3 rounded">
                            <h4 className="font-medium text-blue-800 mb-2">Investment Details</h4>
                            <div className="text-sm space-y-1">
                              <p><strong>Amount:</strong> {test.result.amount}</p>
                              <p><strong>Side:</strong> {test.result.side}</p>
                              <p><strong>Transaction:</strong> {test.result.txHash}</p>
                              {test.result.approvalTx && (
                                <p><strong>Approval TX:</strong> {test.result.approvalTx}</p>
                              )}
                            </div>
                          </div>
                          
                          {/* Updated Prediction Data */}
                          {test.result.updatedPredictionData && !test.result.updatedPredictionData.error && (
                            <div className="bg-green-50 p-3 rounded">
                              <h4 className="font-medium text-green-800 mb-2">Updated Prediction Data</h4>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                <div>
                                  <p className="font-medium">Current Prices:</p>
                                  <p>Yes: {test.result.updatedPredictionData.currentPrices?.yesPrice}</p>
                                  <p>No: {test.result.updatedPredictionData.currentPrices?.noPrice}</p>
                                </div>
                                <div>
                                  <p className="font-medium">Total Liquidity:</p>
                                  <p>{test.result.updatedPredictionData.totalLiquidity}</p>
                                </div>
                                <div>
                                  <p className="font-medium">Your Investment:</p>
                                  <p>Total: {test.result.updatedPredictionData.userInvestment?.totalAmount}</p>
                                  <p>Yes: {test.result.updatedPredictionData.userInvestment?.yesAmount}</p>
                                  <p>No: {test.result.updatedPredictionData.userInvestment?.noAmount}</p>
                                </div>
                                <div>
                                  <p className="font-medium">Prediction Info:</p>
                                  <p>Title: {test.result.updatedPredictionData.predictionInfo?.title}</p>
                                  <p>Yes Liquidity: {test.result.updatedPredictionData.predictionInfo?.yesLiquidity}</p>
                                  <p>No Liquidity: {test.result.updatedPredictionData.predictionInfo?.noLiquidity}</p>
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          {test.result.updatedPredictionData?.error && (
                            <div className="bg-red-50 p-3 rounded">
                              <p className="text-red-700 text-sm">Error getting updated data: {test.result.updatedPredictionData.error}</p>
                            </div>
                          )}
                        </div>
                      ) : (
                  ...
                  {test.gasUsed && (
                    <div>
                      <p className="text-sm text-gray-600">Gas Used: {test.gasUsed}</p>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mt-6">
          ...
          <ul className="text-blue-700 space-y-1">
            <li>1. Connect your wallet and ensure you're on Sepolia testnet</li>
            <li>2. Make sure you have some Sepolia ETH for gas fees</li>
            <li>3. Ensure you have PyUSD tokens on Sepolia for testing</li>
            <li>4. Click "Run All Tests" to execute the complete test suite</li>
            <li>5. Tests will run in logical order: Basic Info → Create → View → Interact</li>
          </ul>
        </div>
      </div>
    </div>
