...
export default function ContractTestPage() {
    ...
    const checkAndApproveTokens = async (amount: string) => {
        ...
        try {
            ...
            console.log(
                `PyUSD Balance: ${ethers.formatUnits(balance, 6)} PyUSD`
            );
    ...
    const getUpdatedPredictionData = async (predictionId: number) => {
        ...
        try {
            ...
            // Get total liquidity
            const totalLiquidity = await contract?.getTotalLiquidity(
                predictionId
            );
            ...
            const predictionWithPrices =
                await contract?.getPredictionWithPrices(predictionId);
        ...
        try {
            ...
            if (predictionCount > 0) {
                ...
                try {
                    ...
                    const prediction = await contract.getPrediction(
                        predictionId
                    );
                ...
                try {
                    ...
                    const prices = await contract.getCurrentPrices(
                        predictionId
                    );
                ...
                try {
                    ...
                    const userInvestments =
                        await contract.getUserInvestmentsInPrediction(
                            account,
                            predictionId
                        );
                    updateTestResult(
    ...
    const testInvestmentFunctions = async () => {
        ...
        try {
            ...
            if (predictionCount > 0) {
                ...
                console.log(
                    `Investing ${formatUSDC(
                        amount
                    )} in prediction ID ${predictionId} (${
                        side ? "Yes" : "No"
                    } side)...`
                );
                const tx = await contract.investInPrediction(
                ...
                console.log(
                    "Getting updated prediction data after investment..."
                );
                const updatedData = await getUpdatedPredictionData(
                    predictionId
                );
            ...
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mt-6">
                    ...
                    <ul className="text-blue-700 space-y-1">
                        <li>
                            1. Connect your wallet and ensure you're on Sepolia
                            testnet
                        </li>
                        <li>
                            2. Make sure you have some Sepolia ETH for gas fees
                        </li>
                        <li>
                            3. Ensure you have PyUSD tokens on Sepolia for
                            testing
                        </li>
                        <li>
                            4. Click "Run All Tests" to execute the complete
                            test suite
                        </li>
                        <li>
                            5. Tests will run in logical order: Basic Info →
