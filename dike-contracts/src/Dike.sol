...
contract MultiversePrediction is ReentrancyGuard, Ownable {
    ...
    function createPrediction(
        ...
        require(pyUSD.transferFrom(msg.sender, address(this), _initialLiquidity), "Transfer failed");
        ...
        emit PredictionCreated(predictionId, msg.sender, _title, _initialLiquidity, _resolutionDate);
    ...
    function investInPrediction(
        ...
        require(block.timestamp < predictions[_predictionId].resolutionDate, "Prediction expired");
        ...
        require(expectedVotes >= _minExpectedVotes, "Slippage tolerance exceeded");
        ...
        emit InvestmentMade(_predictionId, msg.sender, _amount, _side, yesPrice, noPrice);
    }
    
    // ============ GETTER FUNCTIONS ============
    
    function getCurrentPrices(uint256 _predictionId) public view returns (uint256 yesPrice, uint256 noPrice) {
        Prediction memory prediction = predictions[_predictionId];
        uint256 totalLiquidity = prediction.yesLiquidity + prediction.noLiquidity;
    ...
    function getPredictionWithPrices(uint256 _predictionId) external view returns (
    ...
    function getUserChain(address _user) external view returns (
        uint256[] memory predictionIds,
        uint256 totalInvested,
        uint256 totalClaimed
    ) {
        UserChain memory userChain = userChains[_user];
        return (userChain.predictionIds, userChain.totalInvested, userChain.totalClaimed);
    }
    
    function getUserInvestmentsInPrediction(address _user, uint256 _predictionId) 
        external view returns (Investment[] memory) {
        return userInvestments[_user][_predictionId];
    }
    
    function getPredictionInvestments(uint256 _predictionId) 
        external view returns (Investment[] memory) {
        return predictionInvestments[_predictionId];
    }
    
    function getTotalLiquidity(uint256 _predictionId) external view returns (uint256) {
        Prediction memory prediction = predictions[_predictionId];
        return prediction.yesLiquidity + prediction.noLiquidity;
    }
    
    function getActivePredictions() external view returns (uint256[] memory activePredictions) {
    ...
    function getUserTotalInvestmentInPrediction(address _user, uint256 _predictionId) 
        external view returns (uint256 totalAmount, uint256 yesAmount, uint256 noAmount) {
    ...
    function resolvePrediction(uint256 _predictionId, bool _outcome) external onlyOwner {
        ...
        require(block.timestamp >= predictions[_predictionId].resolutionDate, "Cannot resolve before resolution date");
